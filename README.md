...
native-binary, and container gates once. The exact `v0.8.12` tag may publish
...
attestation evidence. See [v0.8.12 release contract](docs/release-v0.8.12.md).
...
historical wide record table. Existing databases are backfilled before
`/readyz` reports ready, newest sessions first, in small transactions. Until
one session is complete the API safely falls back to its original record query;
ingestion and `/healthz` remain available during the index build.
