...
`v0.8.9` succeeds the immutable `v0.8.8` release without moving, deleting, or
reusing an earlier tag or release. It adds bounded, non-sensitive stable-export
materialization telemetry while preserving the existing stable export protocol.

## Identity and gates

The release workflow accepts only `refs/tags/v0.8.9`. Before publishing, it
requires all of the following:

- the tag checkout and `GITHUB_SHA` are the same complete 40-character commit;
- `internal/archive/version.go` declares `0.8.9`;
