...
`v0.8.4` succeeds the immutable `v0.8.3` release without moving, deleting, or
reusing an earlier tag or release. It contains the offline stable-snapshot
metadata covering-index repair.

## Identity and gates

The release workflow accepts only `refs/tags/v0.8.4`. Before publishing, it
requires all of the following:

- the tag checkout and `GITHUB_SHA` are the same complete 40-character commit;
- `internal/archive/version.go` declares `0.8.4`;
