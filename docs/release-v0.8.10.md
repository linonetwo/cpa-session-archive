...
`v0.8.10` succeeds the immutable `v0.8.9` release without moving, deleting, or
reusing an earlier tag or release. It preserves stable export materialization
and keeps repeated HTTP 102 progress responses interim until the verified final
response is ready.

## Identity and gates

The release workflow accepts only `refs/tags/v0.8.10`. Before publishing, it
requires all of the following:

- the tag checkout and `GITHUB_SHA` are the same complete 40-character commit;
- `internal/archive/version.go` declares `0.8.10`;
