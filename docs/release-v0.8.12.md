...
`v0.8.12` succeeds the immutable `v0.8.11` release without moving, deleting, or
reusing an earlier tag or release. It exposes bounded progress records while a
snapshot metadata index is created and adds a compact metadata-only request
mode for refreshing a stable snapshot's idle lease without constructing or
serializing a session page.

The metadata-only mode remains bound to an existing snapshot capability and
its original lower bound, ingest fence, and page limit. It cannot create a
snapshot, accept a pagination cursor, extend the absolute snapshot lifetime,
or change the existing stable pagination response.

## Identity and gates

The release workflow accepts only `refs/tags/v0.8.12`. Before publishing, it
requires all of the following:

- the tag checkout and `GITHUB_SHA` are the same complete 40-character commit;
- `internal/archive/version.go` declares `0.8.12`;
