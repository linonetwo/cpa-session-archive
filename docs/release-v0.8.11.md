...
`v0.8.11` succeeds the immutable `v0.8.10` release without moving, deleting, or
reusing an earlier tag or release. It lets snapshot metadata keepalive requests
proceed while a pinned stable archive export is being materialized, while
preserving the idle and absolute snapshot expiry contracts.

## Identity and gates

The release workflow accepts only `refs/tags/v0.8.11`. Before publishing, it
requires all of the following:

- the tag checkout and `GITHUB_SHA` are the same complete 40-character commit;
- `internal/archive/version.go` declares `0.8.11`;
