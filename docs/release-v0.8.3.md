...
`v0.8.3` succeeds the immutable `v0.8.2` release without moving, deleting, or
reusing an earlier tag or release. It contains the bounded archive stats read
fix for the retained offline migration collector.

## Identity and gates

The release workflow accepts only `refs/tags/v0.8.3`. Before publishing, it
requires all of the following:

- the tag checkout and `GITHUB_SHA` are the same complete 40-character commit;
- `internal/archive/version.go` declares `0.8.3`;
